...
test_time = test_now + timedelta(minutes=2) # 현재 시간에서 2분 더하기

test_date = str(test_date) # 필드 입력을 위해 string로 변환
test_time = str(test_time) # 필드 입력을 위해 string로 변환

test_time = test_time[11:16] # 2분 추가된 시간에서 시간과 분만 저장 # 2022-12-22 17:03:48.128988
...
driver.find_element(By.XPATH, wms_test_result).click() # 딜리버드 상품코드체크박스 xPATH값 조합
...
# 도매매장명 컬럼 선택
driver.find_element(By.CSS_SELECTOR, "div>input[aria-label='필터 컬럼 입력']").send_keys("도매매장명") # 컬럼 검색 필드 - 도매매장명
time.sleep(5)

wms_test_result = driver.find_element(By.CLASS_NAME, 'ag-virtual-list-item.ag-column-select-virtual-list-item') # 체크박스의 상위 그룹의 클래스 네임으로 정보 취득
wms_test_result = wms_test_result.get_attribute("aria-describedby") # 동적으로 변화되는 aria-describedbyD값 취득
wms_test_result = "//*[@id=\"" + wms_test_result + "\"]""" # 컬럼 선택을 위한 각 체크박스 xPATH값 조합
driver.find_element(By.XPATH, wms_test_result).click() # 도매매장명 체크박스 xPATH값 조합
...
# 도매단가 컬럼 선택
driver.find_element(By.CSS_SELECTOR, "div>input[aria-label='필터 컬럼 입력']").send_keys("도매단가") # 컬럼 검색 필드 - 도매단가
time.sleep(5)

wms_test_result = driver.find_element(By.CLASS_NAME, 'ag-virtual-list-item.ag-column-select-virtual-list-item') # 체크박스의 상위 그룹의 클래스 네임으로 정보 취득
wms_test_result = wms_test_result.get_attribute("aria-describedby") # 동적으로 변화되는 aria-describedbyD값 취득
wms_test_result = "//*[@id=\"" + wms_test_result + "\"]""" # 컬럼 선택을 위한 각 체크박스 xPATH값 조합
driver.find_element(By.XPATH, wms_test_result).click() # 도매단가 체크박스 xPATH값 조합
...
# 합계(주문수량) 컬럼 선택
driver.find_element(By.CSS_SELECTOR, "div>input[aria-label='필터 컬럼 입력']").send_keys("주문수량") # 컬럼 검색 필드 - 장끼수량
time.sleep(5)
...
while wms_search_list_row_index_ini >= wms_search_list_row : # 리스트 row 수 만큼 실행
    ...
    #try: # 첫 사입인 도매일 경우 도매 매장명이 나오지 않을 수 있음
    wms_search_list_row_result_check_2 = wms_search_list_row_id_result.find_element(By.CSS_SELECTOR, 'div[aria-colindex="16"]') # 도매처정보(WMS) 컬럼 >  도매매장명 : 16
...
# wms_test_result = driver.find_elements(By.CLASS_NAME, 'MuiButton-root.MuiButton-contained.MuiButton-containedPrimary.MuiButton-sizeSmall MuiButton-containedSizeSmall.MuiButtonBase-root.jss91.css-1ugafsf') # 버튼의 상위 그룹의 클래스 네임으로 정보 취득
wms_test_result = driver.find_elements(By.CLASS_NAME, 'MuiGrid-root.MuiGrid-item.css-1wxaqej') # 버튼의 상위 그룹의 클래스 네임으로 정보 취득
#wms_test_result = wms_test_result.find_elements(By.CLASS_NAME, 'MuiButton-root.MuiButton-contained.MuiButton-containedPrimary.MuiButton-sizeSmall MuiButton-containedSizeSmall.MuiButtonBase-root.jss91.css-1ugafsf') # 버튼의 상위 그룹의 클래스 네임으로 정보 취득
sizeOfDemoList = len(wms_test_result)
print("sizeOfDemoList ", sizeOfDemoList)
wms_test_result1 = wms_test_result[0].get_attribute('innerText')
print("11111", wms_test_result1)
wms_test_result1 = wms_test_result[1].get_attribute('innerText')
print("22222", wms_test_result1)
wms_test_result1 = wms_test_result[2].get_attribute('innerText')
print("33333", wms_test_result1)
wms_test_result1 = wms_test_result[3].get_attribute('innerText')
print("44444", wms_test_result1)

print(wms_test_result[0])
wms_test_result[1].click() # 하단 -> [입고 검수 진행 처리 (소봉 바코드 출력) ] 버튼

'''
try:
    wms_test_result = driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/div[2]/div/div/div[9]/div[2]/button[2]').get_attribute("innerText")
    print("wms_test_result", wms_test_result)
    driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/div[2]/div/div/div[9]/div[2]/button[2]').click() # 하단 -> [입고 검수 진행 처리 (소봉 바코드 출력) ] 버튼
except:
    pass

try:
    wms_test_result = driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/div[2]/div/div/div[8]/div[2]/button[2]').get_attribute("innerText")
    print("wms_test_result", wms_test_result)
...
try:
    wms_test_result = driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/div[2]/div/div/div[7]/div[2]/button[2]').get_attribute("innerText")
    print("wms_test_result", wms_test_result)
    driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/div[2]/div/div/div[7]/div[2]/button[2]').click() # 하단 -> [입고 검수 진행 처리 (소봉 바코드 출력) ] 버튼
except:
    pass
...
wms_search_list_row_all = " ~ " + wms_search_list_row_all # ~ 과 전체 row의 값을 함치기
...
# 검색된 ROWS 데이터 가져오기
wms_test_result = driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/div[2]/div/div/div[4]/div/div[3]/div[1]/div/span[2]')
wms_test_result_check = wms_test_result.text # ex) 3 ~ 9 으로 출력, 검색 된 row 수 ~ 전체 row 수
wms_search_list_row = wms_test_result_check.replace(wms_search_list_row_all,'') # 출력된 row에서 ~ 전체 row 의 문자역 삭제
...
########################################################################################################################
##### 입고 관리 - 입고 확정 #####
driver.find_element(By.XPATH, '//*[@id="root"]/div/div/div/div/div[2]/div/div/div[1]/div/a[5]').click() # 입고 관리 -> 입고 확정 이동
time.sleep(2)
print("##########")
...
# 입고 확정 쇼핑몰 수 가져오기
time.sleep(20) # 데이터 가져오려면 일정 시간 대기를 해야 로드 할 수 있음
...
print("wms_test_result_check1", wms_test_result_check,("1"))
...
print("wms_test_result_check2", wms_test_result_check,("2"))
